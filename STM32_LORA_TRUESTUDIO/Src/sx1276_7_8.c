...
**********************************************************/
int16_t sx1276_7_8_LoRaReadRSSI(void)
{
  uint8_t temp=10;
  temp=SPIRead(LR_RegRssiValue);                  //Read RegRssiValue£¬Rssi value

  return (int16_t)(-164+temp);
}

/**********************************************************
**Name:     sx1276_7_8_LoRaReadPacketRSSI
**Function: Read the RSSI value of packet
**Input:    none
**Output:   temp, RSSI value
**********************************************************/
int16_t sx1276_7_8_LoRaReadPacketRSSI(void)
{
  uint8_t temp=10;
...
u8 sx1276_7_8_LoRaEntryTx(void)
{
  ...
  SPIWrite(REG_LR_PADAC,0x84); //httu edited from 0x87                                   //Tx for 20dBm
  ...
  SPIWrite(LR_RegFifoAddrPtr,addr);                        //RegFifoAddrPtr
	...
	while(1)
	{
		...
		if(temp==strlen((char*)sx1276_7_8Data))
		{
			break;
		}
		if(SysTime>=3)
...
{
 	uint8_t *p=sx1276_7_8Data;

	BurstWrite(0x00, p, strlen((char*)sx1276_7_8Data));
...
void sx1276_7_8_Config(void)
{
  ...
  //lora mode
	...
	BurstWrite(LR_RegFrMsb,sx1276_7_8FreqTbl[Freq_Sel],3);  //write to address 6,7,8 setting frequency parameter

	//setting base parameter
	SPIWrite(LR_RegPaConfig,sx1276_7_8PowerTbl[Power_Sel]); //Setting output power parameter

	SPIWrite(LR_RegOcp,0x2B);        //HTTU change test 23072018 from 0xB- fromRegOcp,Close Ocp
	...
	else
	{
		...
		SPIWrite(LR_RegModemConfig2,((sx1276_7_8SpreadFactorTbl[Lora_Rate_Sel]<<4)+(SPI_CRC<<2)+0x00));  //SFactor &  LNA gain set by the internal AGC loop
	}
	//httu test 21072018
//	SPIWrite(LR_RegModemConfig1,0x72); //default value 125khz,CR=4/5, Implicit header mode.
//	SPIWrite(LR_RegModemConfig2,0x70); //default value SF=7 (128 chips/symbol, signle packet, CRC disable,RX timeout MSB.
	///////////////////////////////////////
	SPIWrite(LR_RegSymbTimeoutLsb,0x64);//HTTU changed from 0xff                   //RegSymbTimeoutLsb Timeout = 0x3FF(Max)

	SPIWrite(LR_RegPreambleMsb,0x00);                       //RegPreambleMsb
	SPIWrite(LR_RegPreambleLsb,8);                      //RegPreambleLsb 8+4=12byte Preamble
	//HTTU test 21072018
//	SPIWrite(LR_RegPreambleMsb,0x00);                       //RegPreambleMsb
//		SPIWrite(LR_RegPreambleLsb,0x8);                    //RegPreambleLsb 8 Preamble
	///////////////////////////////////////
